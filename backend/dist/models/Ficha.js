...
class FichaModel {
    static async create(data) {
        ...
        const result = await db.request()
            .input('ProyectoId', mssql_1.default.Int, proyectoId)
            .input('NombreProyecto', mssql_1.default.NVarChar, data.nombreProyecto)
            .input('HHImplementacion', mssql_1.default.Decimal(10, 2), data.hhImplementacion || 0)
            .input('HHPeriodo', mssql_1.default.Decimal(10, 2), data.hhPeriodo || 0)
            .input('HHPlanificadas', mssql_1.default.Decimal(10, 2), data.hhPlanificadas || 0)
            .input('HHReal', mssql_1.default.Decimal(10, 2), data.hhReal || 0)
        ...
        if (data.recursosIds && data.horasPorRecurso) {
            for (const rId of data.recursosIds) {
                ...
                await db.request()
                    .input('FichaId', mssql_1.default.Int, newFichaId)
                    .input('ProfesionalId', mssql_1.default.Int, Number(rId))
                    .input('HorasAsignadas', mssql_1.default.Decimal(10, 2), horas)
                    .query(`
            ...
            try {
                ...
                if (solicitudId) {
                    ...
                    await db.request()
                        .input('SolicitudId', mssql_1.default.Int, solicitudId)
                    ...
                    for (const rId of data.recursosIds) {
                        ...
                        await db.request()
                            .input('SolicitudId', mssql_1.default.Int, solicitudId)
                            .input('ProfesionalId', mssql_1.default.Int, Number(rId))
                            .input('EstimacionHoras', mssql_1.default.Decimal(10, 2), horas)
                            .input('FechaInicioEstimada', mssql_1.default.Date, data.fechaInicio ? new Date(data.fechaInicio) : null)
                            .input('FechaFinEstimada', mssql_1.default.Date, data.fechaTermino ? new Date(data.fechaTermino) : null)
                            .query(`
        ...
        if (data.responsable && data.nombreProyecto) {
            await db.request()
                .input('NombreProyecto', mssql_1.default.NVarChar, data.nombreProyecto)
                .input('Responsable', mssql_1.default.NVarChar, data.responsable)
                .query(`
    ...
    static async update(id, data) {
        ...
        if (data.hhImplementacion !== undefined) {
            ...
            request.input('HHImplementacion', mssql_1.default.Decimal(10, 2), data.hhImplementacion);
        }
        if (data.hhPeriodo !== undefined) {
            fields.push('HHPeriodo = @HHPeriodo');
            request.input('HHPeriodo', mssql_1.default.Decimal(10, 2), data.hhPeriodo);
        }
        ...
        if (data.estado) {
            ...
            request.input('Estado', mssql_1.default.NVarChar, data.estado);
        }
        if (data.fechaInicio !== undefined) {
            fields.push('FechaInicio = @FechaInicio');
            request.input('FechaInicio', mssql_1.default.Date, data.fechaInicio ? new Date(data.fechaInicio) : null);
        }
        if (data.fechaTermino !== undefined) {
            fields.push('FechaFin = @FechaFin');
            request.input('FechaFin', mssql_1.default.Date, data.fechaTermino ? new Date(data.fechaTermino) : null);
        }
        if (data.descripcion !== undefined) {
            fields.push('Descripcion = @Descripcion');
            request.input('Descripcion', mssql_1.default.NVarChar, data.descripcion);
        }
        if (fields.length > 0) {
            request.input('Id', mssql_1.default.Int, fichaId);
            await request.query(`
        ...
        if (data.recursosIds && data.horasPorRecurso) {
            ...
            await db.request()
                .input('FichaId', mssql_1.default.Int, fichaId)
            ...
            try {
                ...
                if (nombreProyecto) {
                    const solicitudResult = await db.request()
                        .input('NombreProyecto', mssql_1.default.NVarChar, nombreProyecto)
                        .query('SELECT Id FROM SolicitudesProyecto WHERE NombreProyecto = @NombreProyecto');
                    const solicitudId = solicitudResult.recordset[0]?.Id;
                    if (solicitudId) {
                        // Eliminar asignaciones existentes en la solicitud
                        ...
                        for (const rId of data.recursosIds) {
                            ...
                            await db.request()
                                ...
                                .input('ProfesionalId', mssql_1.default.Int, Number(rId))
                                .input('EstimacionHoras', mssql_1.default.Decimal(10, 2), horas)
                                .input('FechaInicioEstimada', mssql_1.default.Date, fechaInicio ? new Date(fechaInicio) : null)
                                .input('FechaFinEstimada', mssql_1.default.Date, fechaTermino ? new Date(fechaTermino) : null)
                                .query(`
                                    INSERT INTO ProyectosAsignados (SolicitudId, ProfesionalId, EstimacionHoras, FechaInicioEstimada, FechaFinEstimada, FechaAsignacion, Estado)
                                    VALUES (@SolicitudId, @ProfesionalId, @EstimacionHoras, @FechaInicioEstimada, @FechaFinEstimada, GETDATE(), 'Asignado')
                                `);
        ...
        if (updatedFicha && updatedFicha.nombreProyecto) {
            ...
            if (data.nombreProyecto) {
                ...
                projRequest.input('NombreProyecto', mssql_1.default.NVarChar, data.nombreProyecto);
            }
            if (data.cliente) {
                projectFields.push('Cliente = @Cliente');
                projRequest.input('Cliente', mssql_1.default.NVarChar, data.cliente);
            }
            if (data.lider) {
                projectFields.push('Lider = @Lider');
                projRequest.input('Lider', mssql_1.default.NVarChar, data.lider);
            }
            if (data.venta !== undefined) {
                projectFields.push('Venta = @Venta');
                projRequest.input('Venta', mssql_1.default.Decimal(18, 2), data.venta);
            }
            if (data.avance !== undefined) {
                projectFields.push('Avance = @Avance');
                projRequest.input('Avance', mssql_1.default.Decimal(5, 2), data.avance);
            }
            if (data.estado) {
                projectFields.push('Estado = @Estado');
                projRequest.input('Estado', mssql_1.default.NVarChar, data.estado);
            }
            if (projectFields.length > 0) {
                ...
                if (pId) {
                    projRequest.input('Id', mssql_1.default.Int, pId);
    ...
    static async delete(id) {
        ...
        const fichaResult = await db.request()
            .input('Id', mssql_1.default.Int, fId)
        ...
        await db.request().input('FichaId', mssql_1.default.Int, fId).query('DELETE FROM FichasRecursos WHERE FichaId = @FichaId');
        // Borrar ficha
        await db.request()
            .input('Id', mssql_1.default.Int, fId)
...
exports.FichaModel = FichaModel;
exports.default = FichaModel;
