...
router.get('/token/:token', async (req: Request, res: Response) => {
    try {
        ...
        const solicitud = await SolicitudModel.findByToken(req.params.token);
        if (!solicitud) {
...
router.post(
    ...
    [
        ...
        body('nombreSolicitante').notEmpty().withMessage('El nombre del solicitante es requerido')
    ],
...
router.put('/:id', async (req: Request, res: Response) => {
    try {
        ...
        const solicitudActualizada = await SolicitudModel.update(id, datos);
...
router.post(
    ...
    authMiddleware,
    [
        body('completadoPor').optional().isString().withMessage('completadoPor debe ser texto')
    ],
    async (req: Request, res: Response) => {
        try {
            const { id } = req.params;
            ...
            const solicitudAprobada = await SolicitudModel.update(id, {
                estado: 'Aprobado',
...
router.post(
    ...
    authMiddleware,
    [
        body('motivoRechazo').notEmpty().withMessage('El motivo de rechazo es requerido'),
        body('completadoPor').optional().isString().withMessage('completadoPor debe ser texto')
    ],
    async (req: Request, res: Response) => {
        try {
            ...
            const solicitudRechazada = await SolicitudModel.update(id, {
                estado: 'Rechazado',
