...
app.post(
  ...
  async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      const userId = uuidv4();
      await db.run(
        'INSERT INTO users (id, nombre, username, email, password, empresa, fechaRegistro) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [userId, nombre, username.toLowerCase(), email.toLowerCase(), hashedPassword, empresa || '', new Date().toISOString()]
      );
...
app.post(
  ...
  async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      const user = await db.get(
        'SELECT * FROM users WHERE LOWER(nombre) = LOWER(?) OR LOWER(username) = LOWER(?) OR LOWER(email) = LOWER(?)', 
...
app.post(
  '/api/reset-password-by-username',
  [
    body('username').notEmpty().withMessage('Usuario requerido'),
    ...
    try {
      ...
      if (!transporter) {
        ...
        await db.run(
          ...
          [uuidv4(), email.toLowerCase(), resetToken, expiresAt.toISOString()]
        );
  ...
  async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      const resetToken = await db.get(
        'SELECT * FROM reset_tokens WHERE email = ? AND token = ? AND used = 0 AND expires_at > ?',
...
app.get('/api/users', async (req: Request, res: Response): Promise<Response> => {
  try {
    const db = await getDb();
